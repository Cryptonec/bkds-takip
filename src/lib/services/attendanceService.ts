...
export async function recalculateAttendance(tarih: Date, organizationId: string, now: Date = new Date()): Promise<void> {
  ...
  for (const lesson of lessons) {
    const bkds = lesson.studentId
      ? await prisma.bkdsAggregate.findFirst({
          where: { organizationId, studentId: lesson.studentId, tarih: dateOnly },
        })
      : null;
...
export async function recalculateStaffAttendance(tarih: Date, organizationId: string, now: Date = new Date()): Promise<void> {
  const dateOnly = new Date(tarih);
  dateOnly.setHours(0, 0, 0, 0);
