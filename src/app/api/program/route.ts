...
import { normalizeDerslik, parseTarihSaat } from '@/lib/utils/normalize';
...
export async function POST(req: NextRequest) {
  ...
  const { studentId, staffId, tarih, baslangicSaati, bitisSaati, derslik, bkdsRequired } = body;

  if (!studentId || !staffId || !tarih || !baslangicSaati || !bitisSaati || !derslik) {
    return NextResponse.json({ error: 'Eksik alan' }, { status: 400 });
  }

  const { normalized: derslikNorm } = normalizeDerslik(derslik);
  ...
  const existing = await prisma.lessonSession.findFirst({
    where: { studentId, staffId, baslangic, bitis, organizationId },
  });
  if (existing) return NextResponse.json({ error: 'Bu ders zaten mevcut' }, { status: 409 });

  // LessonSession oluştur
  ...
  // StaffSession — aynı zaman dilimi varsa kullan, yoksa oluştur
